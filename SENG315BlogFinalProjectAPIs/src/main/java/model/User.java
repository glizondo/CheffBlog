...
public class User {
	...
	public JSONArray listUsers() {
		...
		String sqlString = "SELECT * " + "FROM " + mssqlConnection.getDatabase() + ".dbo.users WITH (NOLOCK) ";
	...
	public String updateUser() {
		...
		try {
			...
			String updateUser = "UPDATE " + mssqlConnection.getDatabase() + ".dbo.users SET "
					+ "firstName=IsNull(NullIf(?,''),firstName), lastName=IsNull(NullIf(?,''),lastName), email=IsNull(NullIf(?,''),email),"
					+ "username=IsNull(NullIf(?,''),username), password=IsNull(NullIf(?,''),password), active=IsNull(?,active), userImage=IsNull(NullIf(?,''),userImage) "
					+ "WHERE userID=" + userID + "";

			PreparedStatement ps = connection.prepareStatement(updateUser);

			// Use this to avoid problems with '' from Service
	...
	public String addUser() {
		...
		try {
			...
			String addUser = "INSERT " + mssqlConnection.getDatabase() + ".dbo.users "
					+ "(firstName, lastName, email, username, password, active, userImage) VALUES (?,?,?,?,?,?,?)";
	...
	public JSONObject authenticate() {
		...
		try {
			...
			java.sql.Statement stmt = connection.createStatement(java.sql.ResultSet.TYPE_SCROLL_INSENSITIVE,
